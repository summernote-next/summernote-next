...
describe('icons-svg detectIconBaseUrl', () => {
  ...
  it('falls back to the default font path when no document-like host is available', () => {
    expect(__detectIconBaseUrl__(undefined)).to.equal('font/icons/');
    expect(__detectIconBaseUrl__(null)).to.equal('font/icons/');
  });

  it('falls back when the host object has no script discovery method', () => {
    expect(__detectIconBaseUrl__({})).to.equal('font/icons/');
  });
  ...
  it('derives the icon base url from a summernote-next.js script tag', () => {
    ...
    expect(__detectIconBaseUrl__(host)).to.equal('https://cdn.example.com/dist/font/icons/');
  });
  ...
  it('derives the icon base url from a summernote-next-classic.min.js script tag', () => {
    ...
    expect(__detectIconBaseUrl__(host)).to.equal('https://cdn.example.com/v2/font/icons/');
  });
  ...
  it('ignores script tags that do not look like the bundled editor', () => {
    ...
    expect(__detectIconBaseUrl__(host)).to.equal('font/icons/');
  });
