...
var summernote = (function() {
	...
	*/
	...
	var SURFACE_SELECTORS = ".note-editor, .note-popover, .note-modal, .note-air-popover, .note-image-popover, .note-video-popover, .note-table-popover, .note-link-popover, .note-hint-popover";
	function resolveDarkModeOption(value) {
		...
		if ([
			...
			"system",
			"media"
		].indexOf(normalized) !== -1) return normalized;
	...
	function ancestorHasClass($node, className) {
		...
		if (typeof element.closest === "function") return !!element.closest("." + className);
	...
	function ancestorMatchesSelector($node, selector) {
		if (!$node || !$node.length || typeof document === "undefined") return false;
		const element = $node[0];
		if (typeof element.closest === "function" && element.closest(selector)) return true;
		return false;
	}
	var PAGE_DARK_ATTRIBUTES = [
		"data-bs-theme",
		"data-theme",
		"data-mode",
		"data-color-scheme",
	...
	function hasExplicitPageTheme() {
		...
		for (let i = 0; i < PAGE_DARK_ATTRIBUTES.length; i++) {
			...
			if (htmlValue === "dark" || bodyValue === "dark") return "on";
			if (htmlValue === "light" || bodyValue === "light") return "off";
		}
		for (let i = 0; i < PAGE_DARK_CLASSES.length; i++) {
			const cls = PAGE_DARK_CLASSES[i];
			if (html && html.classList.contains(cls) || body && body.classList.contains(cls)) return "on";
		}
		for (let i = 0; i < PAGE_DARK_CLASSES.length; i++) {
			const cls = PAGE_DARK_CLASSES[i];
			const lightCls = cls === "dark" ? "light" : cls === "dark-mode" ? "light-mode" : "theme-light";
			if (html && html.classList.contains(lightCls) || body && body.classList.contains(lightCls)) return "off";
		}
		return null;
	}
	function detectInitialMode(resolved, $editor) {
		...
		if (resolved === "media" || resolved === "system") return isMediaDark() ? "on" : "off";
		if (resolved && resolved.kind === "class") return ancestorHasClass($editor, resolved.className) ? "on" : "off";
		if (resolved && resolved.kind === "selector") return ancestorMatchesSelector($editor, resolved.selector) ? "on" : "off";
	...
	function applyModeToNode($node, mode) {
		...
		if (mode === "on") $node.addClass(DARK_CLASS);
		...
		const observer = new MutationObserver((mutations) => {
			for (let i = 0; i < mutations.length; i++) {
				...
				for (let j = 0; j < added.length; j++) {
					...
					if (node.matches && node.matches(SURFACE_SELECTORS)) callback($$(node));
					else if (node.querySelectorAll) node.querySelectorAll(SURFACE_SELECTORS).forEach((descendant) => callback($$(descendant)));
				}
			}
		});
	...
	*/
	var Theme = class {
		...
		initialize() {
			...
			if (this._needsMediaWatch()) this._watchMedia();
			if (this._needsClassWatch() || this.resolved === "auto") this._watchDocumentAttributes();
		...
		apply(resolvedMode) {
			const mode = detectInitialMode(resolvedMode || this.resolved, this.$editor);
			...
			} else applyModeToNode(this.$editor, "auto");
		}
		setMode(mode) {
			const resolved = resolveDarkModeOption(typeof mode === "string" ? mode.trim().toLowerCase() : "");
		...
		_watchMedia() {
			...
			if (typeof this._mediaQueryList.addEventListener === "function") this._mediaQueryList.addEventListener("change", this._mediaListener);
			else if (typeof this._mediaQueryList.addListener === "function") this._mediaQueryList.addListener(this._mediaListener);
		}
		_unwatchMedia() {
			...
			if (typeof this._mediaQueryList.removeEventListener === "function") this._mediaQueryList.removeEventListener("change", this._mediaListener);
			else if (typeof this._mediaQueryList.removeListener === "function") this._mediaQueryList.removeListener(this._mediaListener);
			this._mediaQueryList = null;
			this._mediaListener = null;
		}
		_watchDocumentAttributes() {
			...
			this._documentObserver.observe(document.body, { attributes: true });
		}
	...
	*/
	...
	var icon = function(iconClassName, tagName) {
		if (iconClassName.match(/^</)) return iconClassName;
		tagName = tagName || "i";
	...
	//#endregion
	//#region src/styles/classic/summernote-next-classic.js
	if ($$.summernote && $$.summernote.options && $$.summernote.options.modules) $$.summernote.options.modules.theme = Theme;
